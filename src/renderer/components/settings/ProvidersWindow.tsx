...
function AuthBadge({ provider, t }: { provider: ProviderInfo; t: (k: string) => string }) {
	...
	if (provider.authKind === "oauth") return <Badge variant="success" dot>{t("providers.badge.oauth")}</Badge>;
	if (provider.authKind === "env") return <Badge variant="info">{t("providers.badge.env")}</Badge>;
	return <Badge variant="success" dot>{t("providers.badge.apikey")}</Badge>;
}

function ProviderRow({ provider, onLogin, onLogout, busy, t }: {
		...
		<div className="flex items-center gap-3 rounded-lg border border-[var(--omp-border-muted)] bg-[var(--omp-bg-secondary)] px-3 py-2.5">
			...
			<div className="flex shrink-0 items-center gap-1.5">
				{provider.oauth && !provider.authenticated && (
					<Button size="sm" variant="primary" icon={<LogIn size={12} />} disabled={busy} onClick={() => onLogin(provider.id)}>
						{t("providers.login")}
					</Button>
				)}
				{provider.authenticated && (
					<Button size="sm" variant="ghost" icon={<LogOut size={12} />} disabled={busy} onClick={() => onLogout(provider.id)}>
						{t("providers.logout")}
					</Button>
				)}
			</div>
			...
			<div className="flex max-h-[70vh] flex-col gap-4 overflow-y-auto">
				...
				{authenticated.length > 0 && (
					<div className="flex flex-col gap-2">
						{authenticated.map(p => (
							<ProviderRow key={p.id} provider={p} onLogin={handleLogin} onLogout={handleLogout} busy={busyProvider === p.id} t={t} />
						))}
					</div>
				)}
				...
				<div className="rounded-md border border-[var(--omp-border-muted)] bg-[var(--omp-bg-secondary)] px-3 py-2.5">
					...
					<div className="text-[10.5px] leading-[1.5] text-[var(--omp-muted)]">
						{t("providers.customHelp", { file: "~/.omp/models.yml", baseUrl: "baseUrl", apiKey: "apiKey", models: "models" })}
					</div>
				</div>
			</div>
		</Modal>
